...
function drawEditor(ctx: CanvasRenderingContext2D, state: RenderState) {
  ...
  for (let i = startLine; i < endLine; i++) {
    ...
    const isVisual =
      state.mode === "VISUAL LINE" && state.visualStartLine !== null;
    const isSelected =
      isVisual &&
    ...
    } else if (isCursorLine && state.mode !== "VISUAL LINE") {
      ctx.fillStyle = "#1f1d2e";
      ctx.fillRect(EDITOR_LEFT, y - 24, EDITOR_WIDTH, LINE_HEIGHT);
    }
    ...
    // cursor (block)
    if (isCursorLine && state.mode !== "VISUAL LINE") {
      const cursorX =
...
function drawStatusBar(ctx: CanvasRenderingContext2D, state: RenderState) {
  ...
  if (state.mode === "VISUAL LINE") bg = C.gold;
